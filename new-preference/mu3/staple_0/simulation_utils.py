...
def new_search_points(point, history, xgrid, ygrid):
    ...
    # Categorize history points based on their relationship to the novel point
    dominant_or_dominated = []  # Points where (x, y) >= (x', y') or (x', y') >= (x, y)
    mixed = []  # Points where one coordinate is higher in one and one is higher in the other
    for c in history + [(0, 0)]:
        if (c[0] >= point[0] and c[1] >= point[1]) or (point[0] >= c[0] and point[1] >= c[1]):
            dominant_or_dominated.append(c)
        else:
            mixed.append(c)
    ...
    # Handle dominant or dominated points
    for c in dominant_or_dominated:
        newcandidates.append((point[0], c[1]))  # (x', y)
        newcandidates.append((c[0], point[1]))  # (x, y')

    # Handle mixed points
    for c in mixed:
        newcandidates.append((min(c[0], point[0]), min(c[1], point[1])))
...
def generate_initial_points(startx=0.0, endx=0.5, starty=0.0, endy=0.5, num_points=300):
    """
...
def is_frontier(point, searched_points):
    """Check if a point is on the frontier."""
    for other_point in searched_points:
        # If there's a point with both x and y greater or equal,
        # then 'point' is not on the frontier.
        if other_point[0] >= point[0] and other_point[1] >= point[1] and other_point != point:
            return 'nf'
    return 'f'

def count_switches(frontier):
    """Count the switches from 'f' to 'nf' and from 'nf' to 'f'."""

    # Count the number of switches between 'f' and 'nf'
    switches = sum(1 for i in range(1, len(frontier)) if frontier[i] != frontier[i-1])
